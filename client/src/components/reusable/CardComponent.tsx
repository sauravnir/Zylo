...
export function ProductCard({
  isSearchContent,
  ...props
}: ProductCardProps & { isSearchContent?: boolean }) {
  // Handle Image Hover
  const hoverImage =
    props.images && props.images.length > 1 ? props.images[1] : null;
  ...
  return (
    <motion.div variants={itemVariants} className="w-full">
      ...
      >
        <Card
          className={`border-none group bg-transparent shadow-none rounded-none overflow-hidden ${
        ...
        >
          <CardContent
            ...
            className={`p-0 relative ${isSearchContent ? "h-full md:aspect-[1/1]" : "aspect-[1/1]"}`}
          >
            ...
            {props.availability !== "Sold Out" && !isSearchContent && (
              ...
              >
                ...
                >
                  <Button
                    variant="ghost"
                    className="!w-16 !h-16 !min-w-[64px] !min-h-[64px] p-0 bg-none rounded-none flex-shrink-0"
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsOpenModal(true);
                    }}
                  >
                    <Plus
                      className="!w-5 !h-5 text-main transition-transform duration-300 ease-in-out hover:rotate-90"
                      strokeWidth={2}
                    />
                  </Button>
                </motion.div>
            ...
            {props.availability !== "Sold Out" && !isSearchContent && (
              <div className="md:hidden absolute bottom-2 right-2 z-20 p-2">
                <Button
                  variant="ghost"
                  className="w-8 h-8 bg-background rounded-none"
                  onClick={(e) => {
                    e.stopPropagation();
                    setIsOpenModal(true);
                  }}
        ...
        >
          <h1
            className={`font-body text-main uppercase font-semibold ${
              isSearchContent
                ? "text-[10px] md:text-product-title line-clamp-1"
                : "text-product-title"
            }`}
          >
            {props.title}
          ...
          <span
            className={`text-main/70 uppercase ${
              isSearchContent
                ? "text-[10px] md:text-product-title"
                : "text-product-title"
            }`}
          >
